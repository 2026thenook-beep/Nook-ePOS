...
assert(app.includes("if (tab === 'Till' && !state.cart.length) {"),'Till entry must identify an empty order');
assert(app.includes("state.ticketMeta.OrderType = '';"),'empty Till entry must clear stale order type');
assert(app.includes("if (tab === 'Till' && !state.cart.length) scheduleOrderTypePrompt('till');"),'Till selection prompt must be scheduled for every empty order');
...
assert(app.includes("scheduleOrderTypePrompt('post-payment')"),'guarded post-payment popup scheduling missing');
...
console.log('3.13.18 guided order type workflow verified.');
